...
def generate(test_data, model, tokenizer, args):
    ...
    for feature in tqdm(test_data):
        ...
        gen = model.generate(max_length=args.max_len_generate,
                            ...
                            **content)
        gen = tokenizer.batch_decode(gen, skip_special_tokens=True)
        # print("gen:", gen) # ['工程 投资 管控 不 到位 ， 设计 审核 把关 不严 ， 设计 批复 金额 与 转 固 金额 差异 较大 。 通过 在建 工程 明细 总 表 、 设计 批复 文件 等 ， 审计 发现 分公司 任期 内 部分 工程设计 批复 金额 与 转 固 金额 差异 较大 。']
        # assert 0
        gen = [item.replace(' ', '') for item in gen] # 对分词之间的空格进行删除
        ...
        # print('gen[0]:', gen[0])
        # assert 0
        source, target = ' '.join(jieba.cut(gen[0], cut_all=False)), ' '.join(jieba.cut(feature['summary'][0], cut_all=False))
...
def init_argument():
    ...
    parser.add_argument('--test_data', default='data_utils/data0805/Ques/test.json')
    # parser.add_argument('--result_file', default='filtered_data/test_result.tsv')
    parser.add_argument('--result_file', default='data_utils/data0805/Ques/test_result.json')
    parser.add_argument('--pretrain_model', default='t5_pegasus_pretrain')    
    parser.add_argument('--model', default='saved_model/Ques_0805/summary_model')

    parser.add_argument('--batch_size', default=1, help='batch size') # 16->1
    parser.add_argument('--max_len', default=1320, help='max length of inputs')
    parser.add_argument('--max_len_generate', default=1320, help='max length of generated text')
...
# {'rouge-1': 0.5240770646863862, 'rouge-2': 0.3985619492586921, 'rouge-l': 0.5058518523074719}
# 调整重新生成：{'rouge-1': 0.5240770646863862, 'rouge-2': 0.3985619492586921, 'rouge-l': 0.5058518523074719}